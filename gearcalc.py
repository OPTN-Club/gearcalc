...
#input data
final_drive = 3.9
gear_ratios = [2.7, 2.11, 1.7, 1.39, 1.16, .99, .85]
...
# Make the plot
p = figure(plot_height=400, plot_width=1200, title="Gear Ratio Tuner")
source = None
slider = []
for idx, gear in enumerate(gear_ratios):
    gear_adjusted_wheel_speeds = [wheelspeed/(gear*final_drive) for wheelspeed in wheelspeeds]
    gear_adjusted_torque = [torque*(gear*final_drive) for torque in torque_curve]
    source = ColumnDataSource(data=dict(x=gear_adjusted_wheel_speeds, y=gear_adjusted_torque))
    p.line('x','y', source=source, color=palette[idx], line_width=3)
